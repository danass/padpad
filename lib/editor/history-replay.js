...
export function replayHistory(snapshot, events) {
  ...
  // Strategy: Always prefer the most recent content source
  // 1. Check events first (most recent changes)
  // 2. Fall back to snapshot if events don't have content
  // 3. Default to empty doc if nothing found
  
  // First, try to find content in events (most recent first)
  ...
  // If no content found in events, try snapshot
  if (!currentContent || (currentContent.type === 'doc' && 
    ...
    if (snapshot && snapshot.content_json) {
      ...
      // Use snapshot if it has valid content
      if (snapshotContent && snapshotContent.type === 'doc' && 
          Array.isArray(snapshotContent.content) && snapshotContent.content.length > 0) {
