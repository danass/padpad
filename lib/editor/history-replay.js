...
export function replayHistory(snapshot, events) {
  ...
  if (events && events.length > 0) {
    ...
    for (let i = events.length - 1; i >= 0; i--) {
      ...
      // Check if payload has content directly or nested
      if (payload && payload.content) {
        // Verify it's valid content structure
        const contentToCheck = payload.content
        if (contentToCheck && (
          contentToCheck.type === 'doc' || 
          (Array.isArray(contentToCheck) && contentToCheck.length > 0) ||
          (typeof contentToCheck === 'object' && contentToCheck.content && Array.isArray(contentToCheck.content) && contentToCheck.content.length > 0)
        )) {
          // If it's already a doc structure, use it directly
          if (contentToCheck.type === 'doc') {
            currentContent = contentToCheck
          } else {
            // Otherwise wrap it
            currentContent = {
              type: 'doc',
              content: Array.isArray(contentToCheck) ? contentToCheck : [contentToCheck]
            }
          }
          break
        }
